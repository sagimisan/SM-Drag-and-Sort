...
export default class AnySizeDragSortableView extends React.PureComponent {
  ...
  constructor(props) {
    ...
    this.prevLayoutSnapshot = new Map();
  ...
  moveTouch = (nativeEvent, gestureState) => {
    ...
    if (!this.isUpdating) {
      ...
      // When sliding to the end of next line
      ...
      // When dragging into empty space on any row, find the closest item on that row
      if (!this.isUpdating) {
        const moveCenterY = (moveY1 + moveY2) / 2;
        let closestRowLastItem = null;
        for (let layout of this.layoutMap.values()) {
          if (layout.key === curLayout.key) continue;
          // Check if dragged item center is vertically within this item's row
          if (moveCenterY >= layout.y && moveCenterY <= layout.y + layout.height) {
            // Track the rightmost item on this row
            if (!closestRowLastItem || layout.x + layout.width > closestRowLastItem.x + closestRowLastItem.width) {
              closestRowLastItem = layout;
            }
          }
        }
        // If we found a row and dragging past the last item on it
        if (closestRowLastItem && moveX1 >= closestRowLastItem.x + closestRowLastItem.width) {
          this.move(curLayout.key, closestRowLastItem.key, vy, curLayout.y !== closestRowLastItem.y)
        }
      }
    }
  ...
  move = (fromKey, toKey, vy, isDiffline) => {
    ...
    // Snapshot all current positions before the swap
    this.prevLayoutSnapshot = new Map();
    for (let [key, layout] of this.layoutMap) {
      this.prevLayoutSnapshot.set(key, {...layout});
    }
  ...
  getAnimatedValue = (key) => {
    if (!this.animatedValues.has(key)) {
      this.animatedValues.set(key, new Animated.ValueXY({x: 0, y: 0}));
    }
  ...
  _setLayoutData = (key, event) => {
    ...
    this.layoutMap.set(key, newLayout);

    // FLIP animation: if we're in a swap, animate from old position to new
    if (this.isSwapping) {
      const prevLayout = this.prevLayoutSnapshot.get(key);
      if (prevLayout) {
        const dx = prevLayout.x - newLayout.x;
        const dy = prevLayout.y - newLayout.y;
        if (Math.abs(dx) > 1 || Math.abs(dy) > 1) {
          const animValue = this.getAnimatedValue(key);
...
AnySizeDragSortableView.defaultProps = {
  areaOverlapRatio: 0.25, // Lower = faster swaps
  autoThrottle: 2,
